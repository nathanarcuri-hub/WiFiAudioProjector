class ReceiverPcmQueueProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
    this.channels = 2;
    this.queue = [];
    this.queuedSamples = 0;
    this.maxBufferedSamples = this.samplesFor(84);
    this.targetBufferedSamples = this.samplesFor(52);
    this.startBufferedSamples = this.samplesFor(30);
    ...
    this.port.onmessage = (event) => {
      ...
      if (type === 'config') {
        ...
        this.maxBufferedSamples = this.samplesFor(maxBufferMs ?? 84);
        this.targetBufferedSamples = this.samplesFor(targetBufferMs ?? 52);
        this.startBufferedSamples = this.samplesFor(startBufferMs ?? 30);
  ...
  fillGap(output, channelCount, frameCount, startFrame = 0, startChannel = 0) {
  ...
  process(_inputs, outputs) {
    ...
    if (this.buffering && this.queuedSamples < resumeThreshold) {
      this.fillGap(output, channelCount, frameCount);
    ...
    for (let frame = 0; frame < frameCount; frame += 1) {
      for (let channel = 0; channel < channelCount; channel += 1) {
        ...
        if (sample === null) {
          this.buffering = true;
          this.beginHold();
          this.fillGap(output, channelCount, frameCount, frame, channel);
...
registerProcessor('receiver-pcm-queue-processor', ReceiverPcmQueueProcessor);
