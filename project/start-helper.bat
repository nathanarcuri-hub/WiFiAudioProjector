...
set "HELPER_DLL=%PROJECT_DIR%\native\windows-capture-helper\bin\mode-compare\net8.0-windows\WindowsCaptureHelper.dll"
set "FRAME_DURATION_MS=10"

> "%HELPER_LOG%" echo ==== Helper launch %date% %time% ====

if not exist "%DOTNET_EXE%" (
  echo .NET runtime not found. Run Update WifiAudioProjector Dependencies.bat to install dependencies. >> "%HELPER_LOG%"
  echo .NET runtime not found. Run Update WifiAudioProjector Dependencies.bat to install dependencies.
  endlocal
  exit /b 1
)

if not exist "%HELPER_DLL%" (
  echo Helper DLL not found. Run Update WifiAudioProjector Dependencies.bat to install dependencies. >> "%HELPER_LOG%"
  echo Helper DLL not found. Run Update WifiAudioProjector Dependencies.bat to install dependencies.
  endlocal
  exit /b 1
)
